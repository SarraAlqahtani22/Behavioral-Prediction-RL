...
def create_timeseries(x,y,done):
    ...
    for i in range(9,len(data)):
        ...
        if done[i] == 1:
            last_done = i
            dataX.append(np.vstack((x[i - 9], x[i-8],x[i - 7], x[i - 6], x[i - 5], x[i-4],x[i - 3], x[i - 2], x[i - 1], x[i])))
            dataY.append(y[i])
        ...
        elif done[i] != 1 and i >= last_done - 10:
            dataX.append(np.vstack((x[i - 9], x[i-8],x[i - 7], x[i - 6], x[i - 5], x[i-4],x[i - 3], x[i - 2],x[i - 1],x[i])))
            dataY.append(y[i])
...
model.save('Pend_State_LSTM10.keras')
print(model.summary())

np.save("history_Pend_State_LSTM10.npy", history.history, allow_pickle=True)