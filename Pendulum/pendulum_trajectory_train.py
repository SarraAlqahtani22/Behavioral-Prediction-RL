...
from tensorflow.python.keras.layers import LSTM, Dense, Input,Reshape
...
def create_timeseries(x,y,done):
    ...
    for i in range(3,len(data)):
        ...
        if done[i] == 1:
            ...
            dataY.append(np.vstack((y[i - 3], y[i - 2], y[i - 1], y[i])))
...
model.add(LSTM(80))
model.add(Dense(80))
model.add(Dense(valY.shape[1]*4))
model.add(Reshape((4,valY.shape[1])))
...
model.save('Pend_Trajectory.keras')
print(model.summary())

np.save("history_Pend_Trajectory.npy", history.history, allow_pickle=True)
